...
public:
    ...
    void inorderTraversal(Node* root, vector<int>& result) {
        ...
        result.push_back(root->data);          // Store the root node's data
    ...
    vector<int> mergeSortedArrays(const vector<int>& arr1, const vector<int>& arr2) {
        ...
        // Merge the two arrays using the two-pointer technique
        while (i < n1 && j < n2) {
            if (arr1[i] <= arr2[j]) {
    ...
    // Main function to return a list of integers denoting the node
    // values of both the BSTs in sorted order.
    vector<int> merge(Node *root1, Node *root2) {
        // Step 1: Perform in-order traversal to get the sorted elements from both BSTs
