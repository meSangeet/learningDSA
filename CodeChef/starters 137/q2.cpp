...
void solve() {
    ...
    if((x[i+1]-x[i]) <= P[i+1] && (x[i+1]-x[i]) > P[i])
    {
        while(i<n-1 && (x[i+1]-x[i]) <= P[i+1]) i++;
        i++;
        while(i<(n-1))
        {
            if((x[i+1]-x[i]) <= min(P[i], P[i+1]))
    ...
    }else if((x[i+1]-x[i]) > P[i+1] && (x[i+1]-x[i]) <= P[i])
    {
        while(i<n-1 && (x[i+1]-x[i]) <= P[i]) i++;
        i++;
        while(i<(n-1))
        {
            if((x[i+1]-x[i]) <= min(P[i], P[i+1]))
